...
func Encode(points []Point) EncodedStreams {
	...
	if len(points) == 0 {
		return EncodedStreams{Timestamps: tsW.flush(), Values: valW.flush()}
	}

	// First point: raw 64-bit ts and float, one writer each.
	tsW.writeBits(points[0].TS, 64)
	valW.writeBits(math.Float64bits(points[0].Value), 64)
	...
	tsW.writeBits(uint64(delta1), 64)

	prevBits := math.Float64bits(points[0].Value)
	var prevLZ, prevMBC uint
	var hasWindow bool
	prevBits, prevLZ, prevMBC, hasWindow = encodeXOR(valW, math.Float64bits(points[1].Value), prevBits, prevLZ, prevMBC, hasWindow)
	...
	for i := 2; i < len(points); i++ {
		...
		prevBits, prevLZ, prevMBC, hasWindow = encodeXOR(valW, curBits, prevBits, prevLZ, prevMBC, hasWindow)
	}
...
func Decode(streams EncodedStreams, count int) ([]Point, error) {
	...
	tsR := newBitReader(streams.Timestamps)
	valR := newBitReader(streams.Values)
	pts := make([]Point, 0, count)

	// First point.
	t0, err := tsR.readBits(64)
	if err != nil {
		return nil, ErrTruncatedInput
	}
	v0, err := valR.readBits(64)
	if err != nil {
		return nil, ErrTruncatedInput
	}
	...
	var prevLZ, prevMBC uint
	var hasWindow bool
	v1, lz, mbc, hw, err := decodeXOR(valR, v0, prevLZ, prevMBC, hasWindow)
	if err != nil {
		return nil, err
	}
	...
	for i := 2; i < count; i++ {
		...
		vBits, lz, mbc, hw, err := decodeXOR(valR, prevVBits, prevLZ, prevMBC, hasWindow)
		if err != nil {
			return nil, err
		}
		prevVBits = vBits
...
func Benchmark() BenchmarkResult {
	...
	raw := n * 16
	comp := Encode(pts)
	compressedBytes := len(comp.Timestamps) + len(comp.Values)
	ratio := float64(raw) / float64(compressedBytes)
	return BenchmarkResult{RawBytes: raw, CompressedBytes: compressedBytes, Ratio: ratio}
}
