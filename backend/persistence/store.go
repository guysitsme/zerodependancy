...
type Store struct {
	...
	// per-series state (mutex, in-memory write buffer, next chunk ID)
	// Concurrent writes to different series proceed in parallel without contention.
	seriesStates sync.Map // map[string]*seriesState
...
func (s *Store) getSeriesState(series string) *seriesState {
	if v, ok := s.seriesStates.Load(series); ok {
		return v.(*seriesState)
	}
	st := &seriesState{}
	actual, _ := s.seriesStates.LoadOrStore(series, st)
	return actual.(*seriesState)
}

// WritePoint appends a (timestamp, value) point to the named series.
// It is safe to call from multiple goroutines concurrently for different series;
// concurrent writes to the SAME series are safely serialized by the per-series lock.
func (s *Store) WritePoint(series string, ts uint64, value float64) error {
	st := s.getSeriesState(series)
	st.mu.Lock()
	defer st.mu.Unlock()
	...
	st.buffer = append(st.buffer, compression.Point{TS: ts, Value: value})

	if shouldRotate(st.buffer) {
		return s.rotateLocked(series, st)
	}
	return nil
}
...
// rotateLocked flushes the active buffer for series to a new chunk file.
// Must be called with st.mu held.
func (s *Store) rotateLocked(series string, st *seriesState) error {
	buf := st.buffer
	if len(buf) == 0 {
		return nil
	}

	streams := compression.Encode(buf)
	tsLen := uint32(len(streams.Timestamps))
	valLen := uint32(len(streams.Values))
	payload := make([]byte, 4+tsLen+valLen)
	binary.BigEndian.PutUint32(payload[0:4], tsLen)
	copy(payload[4:4+tsLen], streams.Timestamps)
	copy(payload[4+tsLen:], streams.Values)
...
func (s *Store) ReadChunk(series, fileName string) ([]compression.Point, error) {
	...
	if len(payload) < 4 {
		return nil, fmt.Errorf("persistence: payload too short in %s", fileName)
	}
	tsLen := int(binary.BigEndian.Uint32(payload[0:4]))
	if len(payload) < 4+tsLen {
		return nil, fmt.Errorf("persistence: corrupted timestamp stream length in %s", fileName)
	}
	streams := compression.EncodedStreams{
		Timestamps: payload[4 : 4+tsLen],
		Values:     payload[4+tsLen:],
	}
	return compression.Decode(streams, count)
}
...
func (s *Store) FlushAllSeries() {
	var names []string
	s.seriesStates.Range(func(key, _ any) bool {
